...
#     # inputs1 = inputs
#     # print(inputs)
#     outputs,outputs_s,outputs_g1, outputs_g2 = model(inputs,inputs,inputs,inputs,modal=mode)
#     outputs = outputs.data.cpu()
#     outputs_s = outputs_s.data.cpu()
#     outputs_g1 = outputs_g1.data.cpu()
#     outputs_g2 = outputs_g2.data.cpu()
#     return outputs,outputs_s,outputs_g1, outputs_g2
...
def extract_features(model, data_loader, print_freq=50,flip=True,mode=0):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            # outputs,outputs_s,outputs_g1, outputs_g2 = extract_cnn_feature(model, imgs,mode)
            flip = fliplr(imgs)
            # print(flip)
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
    ...
    query_modal = ['visible' if cam == 1 else 'thermal' for cam in query_cams]
    gallery_modal = ['visible' if cam == 1 else 'thermal' for cam in gallery_cams]
    # Compute mean AP
