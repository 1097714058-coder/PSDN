...
from clustercontrast.trainers import ClusterContrastTrainer_HSDN
...
def generate_correlations(features, pseudo_labels, cluster_nums, neighbour_num, coe):
    ...
    max_label = pseudo_labels.max().item() + 1
    cluster_nums = max_label

    for i in range(inds.shape[0]):
        correlation = torch.zeros(cluster_nums)
...
def main():
    ...
    log_name = 'regdb_train'
    main_worker_stage2(args,log_name) # Stage 1
...
def main_worker_stage2(args,log_name):
    ...
    model.cuda()
    model = nn.DataParallel(model)#,output_device=1)
    trainer = ClusterContrastTrainer_HSDN(model)
    ...
    train_transformer_rgb2gray = T.Compose([
        color_aug,
        T.Resize((height, width)),
        T.Pad(10),
        T.RandomCrop((height, width)),
    ...
    # transform_thermal2gray = T.Compose([
    #     color_aug,
    #     T.Resize((height, width)),
    #     T.Pad(10),
    #     T.RandomCrop((height, width)),
    ...
    for epoch in range(args.epochs):
        ...
        cluster_features_ir, cluster_nums_ir = generate_cluster_features(pseudo_labels_ir, features_ir_ori)
        cluster_features_rgb, cluster_nums_rgb = generate_cluster_features(pseudo_labels_rgb, features_rgb_ori)
        if epoch >=1:
            args.momentum=0.1
        print('args.momentum',args.momentum)
        memory_ir = ClusterMemory(768, num_cluster_ir, temp=args.temp,smooth=0.1,
                               momentum=args.momentum, use_hard=args.use_hard, num_instances=args.num_instances).cuda()
        memory_rgb = ClusterMemory(768, num_cluster_rgb, temp=args.temp,smooth=0.1,
                               momentum=args.momentum, use_hard=args.use_hard, num_instances=args.num_instances).cuda()
...
######################
        cluster_features_ir_s, cluster_nums_ir_s = generate_cluster_features(pseudo_labels_ir, features_ir_s)
        cluster_features_rgb_s, cluster_nums_rgb_s= generate_cluster_features(pseudo_labels_rgb, features_rgb_s)

        memory_ir_s = ClusterMemory(768, num_cluster_ir, temp=args.temp,smooth=0.1,
                               momentum=args.momentum, use_hard=args.use_hard, num_instances=args.num_instances).cuda()
        memory_rgb_s = ClusterMemory(768, num_cluster_rgb, temp=args.temp,smooth=0.1,
                               momentum=args.momentum, use_hard=args.use_hard, num_instances=args.num_instances).cuda()
        ...
        weights_ir = []
        ir_label=[]
        for i, ((fname, _, cid), label, correlation, weight) in enumerate(zip(sorted(dataset_ir.train), pseudo_labels_ir, correlations_ir_all, weights_ir_all)):
            if label != -1:
        ...
        weights_rgb = []
        rgb_label=[]
        for i, ((fname, _, cid), label, correlation, weight) in enumerate(zip(sorted(dataset_rgb.train), pseudo_labels_rgb, correlations_rgb_all, weights_rgb_all)):
            if label != -1:
        ...
        if epoch >= trainer.cmlabel:
            with torch.no_grad():
                ...
                del rgb_self_sim,smooth_rgb,lp_feat_rgb
            ...
            cluster_features_ir,cluster_nums_ir = generate_cluster_features(pseudo_labels_all, features_all)

            shared_memory = ClusterMemory(768, num_cluster_ir, temp=args.temp,smooth=0.1,
                                          momentum=0.1, use_hard=args.use_hard, num_instances=args.num_instances)#.cuda()
            ...
            cluster_features_ir_s,cluster_nums_rgb_s = generate_cluster_features(pseudo_labels_all, features_all_s)
            shared_memory_s = ClusterMemory(768 , num_cluster_ir, temp=args.temp,smooth=0.1,
                                            momentum=0.1, use_hard=args.use_hard, num_instances=args.num_instances)

            shared_memory_s.features = F.normalize(cluster_features_ir, dim=1).cuda()
        ...
        train_loader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                ...
                                trainset=pseudo_labeled_dataset_rgb, no_cam=args.no_cam,train_transformer=train_transformer_rgb,train_transformer1=train_transformer_rgb2gray)
...
##############################
            ...
            mode='all'
            data_path='/root/autodl-tmp/project/data/regdb/'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--balance', type=float, default=0.7)
    parser.add_argument('--coe', type=float, default=10.0)
    parser.add_argument('--neighbour', type=int, default=20)
